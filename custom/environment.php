<?
	...
	$bigtree["config"]["www_root"] = "http://localhost/roadTrip/"; // "www_root" should be http://www.website.com/location/of/the/site/
	$bigtree["config"]["static_root"] = "http://localhost/roadTrip/"; // "static_root" can either be the same as "www_root" or another domain that points to the same place -i t is used to server static files to increase page load time due to max connections per domain in most browsers.
	$bigtree["config"]["admin_root"] = "http://localhost/roadTrip/admin/"; // "admin_root" should be the location you want to access BigTree's admin from, i.e. http://www.website.com/admin/
	...
	$bigtree["config"]["db"]["name"] = "roadTrip";
